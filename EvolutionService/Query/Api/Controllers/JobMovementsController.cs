...
public class JobMovementsController : ControllerBase
{
    ...
    public JobMovementsController(IEvolutionReadRepository<JobMovementReadModel> repository, ReadDbContext readDbContext)
    {
    ...
    public async Task<IActionResult> GetAll([FromQuery] PaginationRequest pagination, CancellationToken ct)
    {
        ...
        return Ok(BaseResponse<PagedResult<JobMovementReadModel>>.Ok(PagedResult<JobMovementReadModel>.Create(items, pagination.NormalizedPageNumber, pagination.NormalizedPageSize, totalCount)));
    }
    ...
    public async Task<IActionResult> GetById(Guid id, CancellationToken ct)
    {
        ...
        return item is null ? NotFound(BaseResponse<object>.Fail("Job movement not found.")) : Ok(BaseResponse<JobMovementReadModel>.Ok(item));
    }
    ...
    [HttpGet("self")]
    [SwaggerOperation(Summary = "List job movements for the current user.")]
    public async Task<IActionResult> GetSelf(CancellationToken ct)
    {
        ...
        return Ok(BaseResponse<IReadOnlyList<JobMovementReadModel>>.Ok(await _repository.GetByEmployeeAsync(employeeId.Value, ct)));
    }
