...
public class RewardsController : ControllerBase
{
    ...
    private readonly IEvolutionReadRepository<RewardReadModel> _repository;
    private readonly ReadDbContext _readDbContext;

    public RewardsController(IEvolutionReadRepository<RewardReadModel> repository, ReadDbContext readDbContext)
    {
    ...
    [SwaggerOperation(Summary = "List rewards.")]
    public async Task<IActionResult> GetAll([FromQuery] PaginationRequest pagination, CancellationToken ct)
    {
        ...
        return Ok(BaseResponse<PagedResult<RewardReadModel>>.Ok(PagedResult<RewardReadModel>.Create(items, pagination.NormalizedPageNumber, pagination.NormalizedPageSize, totalCount)));
    }

    [HttpGet("{id:guid}")]
    [Authorize(Roles = ReaderRoles)]
    [SwaggerOperation(Summary = "Get a reward by ID.")]
    public async Task<IActionResult> GetById(Guid id, CancellationToken ct)
    {
        var item = await _repository.GetByIdAsync(id, ct);
        return item is null ? NotFound(BaseResponse<object>.Fail("Reward not found.")) : Ok(BaseResponse<RewardReadModel>.Ok(item));
    }

    [HttpGet("employee/{employeeId:guid}")]
    [Authorize(Roles = ReaderRoles)]
    [SwaggerOperation(Summary = "List rewards for an employee.")]
    public async Task<IActionResult> GetByEmployee(Guid employeeId, CancellationToken ct)
        => Ok(BaseResponse<IReadOnlyList<RewardReadModel>>.Ok(await _repository.GetByEmployeeAsync(employeeId, ct)));

    [HttpGet("self")]
    [SwaggerOperation(Summary = "List rewards for the current user.")]
    public async Task<IActionResult> GetSelf(CancellationToken ct)
    {
        ...
        return Ok(BaseResponse<IReadOnlyList<RewardReadModel>>.Ok(await _repository.GetByEmployeeAsync(employeeId.Value, ct)));
    }
