...
namespace ProfileService.Command.Infrastructure.Messaging;

public class ProfileOutboxPublisher : BackgroundService
{
    private readonly IServiceProvider _serviceProvider;
    private readonly IKafkaProducer _producer;
    private readonly ILogger<ProfileOutboxPublisher> _logger;
    private readonly string _topic;
    private readonly TimeSpan _interval;

    public ProfileOutboxPublisher(IServiceProvider serviceProvider, IKafkaProducer producer, IConfiguration configuration, ILogger<ProfileOutboxPublisher> logger)
    {
        _serviceProvider = serviceProvider;
        _producer = producer;
        _logger = logger;
        _topic = configuration["Kafka:Topic"] ?? "profile.events";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var dbContext = scope.ServiceProvider.GetRequiredService<ProfileCommandDbContext>();
            ...
            {
                _logger.LogError(ex, "Error while publishing profile outbox messages.");
            }
